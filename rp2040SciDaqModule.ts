...
namespace rp2040SciDaqModule {
    ...
    export enum SciDaqModuleReturnDataType{
        //blockId="SciDaqModuleReturnValueFloat" block="Value(Float)"
        ValueFloat = 1,
        //blockId="SciDaqModuleReturnValueString" block="Value(String)"
        ValueString = 2,
        //blockId="SciDaqModuleReturnUnitString " block="Unit(UnitString)"
        UnitString = 3
    ...
    //% block="get Interface %port key %key %type" 
    //% weight=80
    export function getPortValue(port: SciDaqModulePort, key: string, type: SciDaqModuleReturnDataType): string{
    ...
    export function getTimeStamp(): string{
        let values = "";
        let length = 0;
        let sendpkt = [CMD_GET_TIMESTAMP, length & 0xFF, (length >> 8) & 0xFF, 0];
        sendPacket(sendpkt, 4 + length);
